...
vec3 vec3_sub(vec3 a, vec3 b) { return (vec3){ a.x - b.x, a.y - b.x, a.z - b.z }; }
...
vec2 project(vec3 p)
{
    p.x /= p.z;
    p.y /= p.z;
    return (vec2){ (p.x + .5f) * SCRSIZE, (p.y + .5f) * SCRSIZE / 2 };
}
    ...
    while (true)
    {
        ...
        for (float theta = 0.f; theta < 2.f * M_PI; theta += tstep)
        {
            ...
            for (float phi = 0.f; phi < 2.f * M_PI; phi += pstep)
            {
                ...
                vec3 ring_center = (vec3){ center.x, center.y + in_r + out_r, center.z };
                vec3 pos = vec3_add(ring_center, matmul(prot, (vec3){ 0.f, out_r, 0.f }));

                // Rotate along outer ring
                vec3 tcenter = (vec3){ center.x, center.y, pos.z };
                pos = vec3_add(tcenter, matmul(trot, vec3_sub(pos, tcenter)));

                // Rotate full donut
                pos = vec3_add(center, matmul(rotz, matmul(roty, matmul(rotx, vec3_sub(pos, center)))));
        ...
        angle = vec3_add(angle, (vec3){ .1f, .08f, .04f });
        refresh();
        usleep(1e4);
    }
